...
# Creates a Matrix for the local SIAC filtering operation
function local_SIAC_Mat(r::Int, l::Int, pp::Int, dx::Float64, LocalNodes::Array{Float64})
    ...
    # knot vector for the BSpline Basis of the employed SIAC Kernel
    knot = [-((r+l)/2)+i  for i=0:(r+l)] 

    EL = ceil(Int,abs(minimum(knot)))   # number of elements in the support to the left of the filtered point
    ER = ceil(Int,abs(maximum(knot)))   # number of elements in the support to the left of the filtered point

    numParam = (ER+EL+1)*pp   # Number of parameters we have to consider for the filtering operation at each point
    # This also implies we can filter only for elements > EL and smaller ER

    KMatLoc = zeros(pp,numParam) # Vectors to multiply with the nodes to PostProcess
    # p Rows for each of the local nodes
    # numParam cols for each nodal value of the function
    ...
    # dx = 1
    # According to formula taking i=0 and each m
    MeshBreaks = dx*collect(-(0.5+EL):(0.5+ER))

    # Compute the Breaks
    Breaks = vcat(MeshBreaks,dx*knot)
    sort!(unique!(Breaks))

    xki = [Ref2Elem(LocalNodes[m],-0.5*dx,0.5*dx) for m=1:pp]
    # display([Lag[k](Ref2Elem(LocalNodes[k],-0.5*dx,0.5*dx)) for k=1:pp])

    for elem=1:length(MeshBreaks)-1     # Loop over elements in filter support
        xL = MeshBreaks[elem]
        xR = MeshBreaks[elem+1]
        
        # display([Lag[k](LocalNodes[k]) for k=1:pp])
   
        for k=1:pp # Loop over basis Functions

            LocalBreaks = filter(y->(y>=xL)&&(y<=xR),sort(vcat(MeshBreaks,dx*knot.-xki[k])))#Breaks)
            for br=1:length(LocalBreaks)-1
                ...
                for m=1:pp  # Loop Over Nodes of u_post
                    KMatLoc[m,(elem-1)*pp+k] += (1/dx)*(h/2)*sum(w[ll]*K((xki[m]-pts_loc[ll])/dx).*Lag[k](Elem2Ref(pts_loc[ll],xL,xR)) for ll=1:npts )
                end
            end
        end
    end
...
function global_SIAC_Mat(p::Int64, numEl::Int64, dx::Float64, LocalNodes::Array{Float64}; boundaryTreatment="none")
    ...
    KMatLoc = local_SIAC_Mat(r,l,pp,dx,LocalNodes)
    nPts = Int(pp*numEl)

    KMat = zeros(Float64,pp*numEl,pp*numEl)

    nE_supp = Int(size(KMatLoc,2)/pp)
    n_bdPts = Int(((nE_supp-1)/2)*pp)


    KMat_pre = zeros(Float64,nPts,nPts + 2*n_bdPts)

    cols = collect(1:size(KMatLoc,2))
    for elem=1:numEl
        KMat_pre[(elem-1)*pp+1:(elem)*pp,cols] = KMatLoc
        cols = cols.+pp
    end
    # Assuming Periodic Boundary
    if boundaryTreatment == "periodic"
        KMat = KMat_pre[:,n_bdPts+1:nPts+n_bdPts] + [zeros(nPts,nPts-n_bdPts) KMat_pre[:,1:n_bdPts]] + [KMat_pre[:,(end-n_bdPts+1):end] zeros(nPts,nPts-n_bdPts) ]
...
function global_SIAC_Mat(p::Int64, Pts::Array; boundaryTreatment="none" )
    ...
    return global_SIAC_Mat(p, numEl, dx, LocalNodes[1]; boundaryTreatment=boundaryTreatment)
...
# Computes Local and Global Evaluation points
# Used to generate Evaluation Points based on a set of local Nodes
function makeNodes(mesh::Mesh, p::Int; type="uniform")
    ...
    #   "GLob"   Gauss-Lobatto points
    #   "uni"   Uniformly distributed points
    ...
    if type == "GLeg"
        LocalNodes, ~ = gauss(p+1,-1,1)    # Gauss-Legendre Nodes
    elseif type == "GLob"
        if p==1
            LocalNodes = [-1.0 1.0]
        elseif p==2
        ...
        elseif p==5
            LocalNodes = [-1.0 -sqrt(1/3+(2/21)*sqrt(7)) -sqrt(1/3-(2/21)*sqrt(7)) 0.0  -sqrt(1/3-(2/21)*sqrt(7)) sqrt(1/3+(2/21)*sqrt(7)) 1.0]
        end
    elseif type == "uni"
    ...
    GlobalNodes = zeros(Float64,(p+1)*mesh.N)
    for e = 1:mesh.N
        for i = 1:(p+1)
            GlobalNodes[(p+1)*(e-1)+i] = Ref2Elem(LocalNodes[i],mesh.grid[e],mesh.grid[e+1])
        end
    end

    return GlobalNodes, LocalNodes
end
...
function PostProcess(x::Float64, H::Float64, r::Int, l::Int, Approx::FE, mesh::Mesh, quad::Quadrature)
    ...
    for j=1:mesh.N
        if (mesh.grid[j]>=xL)&&(mesh.grid[j]<=xR)
            BreakPoints = vcat(BreakPoints, mesh.grid[j])
        end
    end
